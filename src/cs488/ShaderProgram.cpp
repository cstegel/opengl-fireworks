 ...
 */
GLint ShaderProgram::getUniformLocation(const char * uniformName, bool ignoreInvalid)
{
	string uniformStr(uniformName);
	if (cachedUniformLocations.count(uniformStr) > 0)
	{
	...
	cachedUniformLocations[uniformName] = result;

	return result;
}

GLint ShaderProgram::getUniformLocation(const string & uniformName, bool ignoreInvalid)
{
    return getUniformLocation(uniformName.c_str(), ignoreInvalid);
}
