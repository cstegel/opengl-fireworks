...
HumanControlSystem::HumanControlSystem(ecs::EntityManager &entities, InputManager &input)
...
bool HumanControlSystem::Frame(double dtSinceLastFrame)
{
	...
	for (ecs::Entity entity : entities.EntitiesWith<Transform, HumanController>())
	{
		...
		// keyboard controls
		for (auto const &actionKeysPair : entity.Get<HumanController>()->inputMap)
		{
